...
class OrderView(generics.ListCreateAPIView):
	...
	def get_queryset(self):
		...
		elif self.request.user.groups.filter(name='Delivery Crew').exists(): #delivery
			return Order.objects.all().filter(delivery_crew=self.request.user) #only
		else: # manager
			return Order.objects.all()
	...
	def create(self, request, *args, **kwargs):
		...
		if (order_serializer.is_valid()):
			...
			for item in items.values():
				orderitem = OrderItem(order=order,menuitem_id=item['menuitem_id'], price=item['price'], quantity=item['quantity'],)
				orderitem.save()
...
class SingleOrderView(generics. RetrieveUpdateAPIView):
	...
	def update(self, request, *args, **kwargs):
		if self.request.user.groups.count()==0: # Normal user, not belonging to any gr
...
class GroupViewSet(viewsets. ViewSet):
	...
	def list(self, request):
		users = User.objects.all().filter(groups_name='Manager')
...
class DeliveryCrewViewSet (viewsets. ViewSet):
	...
	def list(self, request):
		users  = User.objects.all().filter(groups_name='Delivery Crew')
		items = UserSerializer (users, many=True)
		return Response(items.data)
